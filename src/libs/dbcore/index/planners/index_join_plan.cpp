...
namespace dbcore::index
{
    ...
    std::shared_ptr<scan::i_scan> index_join_plan::open()
    {
        ...
        auto ts = std::dynamic_pointer_cast<record::table_scan>(m_rhs_plan->open());
        if (!ts)
            throw std::runtime_error("Right-hand plan for index_join_plan must be of type 'table_scan'");
            
        auto idx = m_idx_stats->open();
        return std::make_shared<index_join_scan>(sc, idx, m_join_field, ts);
    }
    ...
    std::shared_ptr<record::schema> index_join_plan::schema()
    {
        return m_sch;
    }
}